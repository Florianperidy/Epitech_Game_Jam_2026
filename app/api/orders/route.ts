...
export async function POST(request: Request) {
  try {
    ...
    if (!normalizedSymbol || !orderType || Number.isNaN(parsedAmount) || parsedAmount <= 0) {
      return NextResponse.json({ error: "Invalid input" }, { status: 400 });
    }

    const currentPrices = getLivePrices();
    ...
    if (!targetAsset) {
      targetAsset = { symbol: normalizedSymbol, name: normalizedSymbol, amount: 0, isFiat: false };
    ...
    if (bugRoll < 0.3) {
      ...
      await transactions.insertOne({
        ...
        date: new Date(),
        status: bug.status,
        description: `${orderType.toUpperCase()} ${normalizedSymbol} (BUG)`,
      });

      return NextResponse.json({ error: bug.error, hasBug: true }, { status: 400 });
    }
    ...
    if (orderType === "buy") {
      if (eurAsset.amount < orderValue) {
        return NextResponse.json({ error: "Insufficient EUR balance" }, { status: 400 });
      }
      eurAsset.amount -= orderValue;
      targetAsset.amount += parsedAmount;
    } else if (orderType === "sell") {
      if (targetAsset.amount < parsedAmount) {
        return NextResponse.json({ error: `Insufficient ${normalizedSymbol} balance` }, { status: 400 });
      }
      targetAsset.amount -= parsedAmount;
      eurAsset.amount += orderValue;
    }
    ...
    await portfolios.updateOne(
      { userId: session.user.id },
      { $set: { assets: portfolio.assets, updatedAt: now } }
    );
    ...
    await transactions.insertOne({
      ...
      date: now,
      status: "Completed",
      description: `${orderType.toUpperCase()} ${normalizedSymbol} @ $${price.toFixed(2)}`,
    });

    return NextResponse.json({
      success: true,
      message: `Order filled: ${parsedAmount} ${normalizedSymbol} at $${price.toFixed(2)}`
    });
